...
const STORAGE_KEY = 'cjmbi-dashboard-state';
...
export const useAppStore = create<AppState>()(
  persist(
    ...
    }),
    {
      ...
      partialize: (state) => ({
        // Only persist project and UI state, not performance metrics
      ...
      onRehydrateStorage: () => {
        logger.info(LogCategories.Store, 'Rehydrating state from localStorage');
        return (state, error) => {
          if (error) {
            logger.error(LogCategories.Store, 'Failed to rehydrate state', error);
          } else if (state) {
            logger.info(LogCategories.Store, `State restored: "${state.project.name}"`, {
              widgets: state.project.widgets.length,
              dataSources: state.project.dataSources.length,
            });
          }
        };
      },
      migrate: (persistedState, version) => {
        logger.info(LogCategories.Store, `Migrating state from version ${version} to ${STORAGE_VERSION}`);
